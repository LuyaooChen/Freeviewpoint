...
extern void ImageWarping(cuda::GpuMat srcColorL,
                             ...
                             float alpha);
#if DEBUG_MODE
extern void ImageWarping2(cuda::GpuMat srcColorL,
                             cuda::GpuMat srcDepthL,
                             cuda::GpuMat srcProjMatL,
                             cuda::GpuMat srcColorR,
                             cuda::GpuMat srcDepthR,
...
//extern CalibStruct m_CalibParams[7];

//connect the 8 referenced viewpoint consecutively with visual viewpoint.It's linear.
...
//    }
    ...
    Mat colorImgL, colorImgR, dstImg, depthImg;
//    Mat zeroMap = Mat(colorCap[0].get(CAP_PROP_FRAME_HEIGHT),colorCap[0].get(CAP_PROP_FRAME_WIDTH),CV_8UC3);
    cuda::GpuMat d_colorImgL, d_depthImgL, d_dstL, d_srcProjMat[8], d_dstProjMat, d_dst; //d_ means device_
    cuda::GpuMat d_colorImgR, d_depthImgR, d_dstR;
    d_dstL= cuda::GpuMat(IMG_ROWS, IMG_COLS, CV_8UC3);
    d_dstR= cuda::GpuMat(IMG_ROWS, IMG_COLS, CV_8UC3);
    d_dst = cuda::GpuMat(IMG_ROWS, IMG_COLS, CV_8UC3);
    for(int i=0; i<8;i++)
    {
        d_srcProjMat[i].upload(m_CalibParams[i].m_ProjMatrix);
    }
...
//            resize(colorImgL, colorImgL, Size(IMG_COLS,IMG_ROWS));
            d_colorImgL.upload(colorImgL);

            colorImgR = imread(IMG_PATH+to_string(camR)+"/color-cam"+to_string(camR)+"-f"+f_num.str()+".jpg");
//            resize(colorImgR, colorImgR, Size(IMG_COLS,IMG_ROWS));
            d_colorImgR.upload(colorImgR);

            depthImg = imread(IMG_PATH+to_string(camL)+"/depth-cam"+to_string(camL)+"-f"+f_num.str()+".png", IMREAD_GRAYSCALE);
//            resize(depthImg, depthImg, Size(IMG_COLS,IMG_ROWS));
            d_depthImgL.upload(depthImg);

            depthImg = imread(IMG_PATH+to_string(camR)+"/depth-cam"+to_string(camR)+"-f"+f_num.str()+".png", IMREAD_GRAYSCALE);
//            resize(depthImg, depthImg, Size(IMG_COLS,IMG_ROWS));
            d_depthImgR.upload(depthImg);
...
//            stopTime1 = clock();
    ...
    //        cout<<"elemsize:"<<depthImg_t.elemSize()<<" channels:"<<depthImg_t.channels()<<endl;

            ImageWarping(d_colorImgL,
                         d_depthImgL,
                         d_srcProjMat[camL],
                         d_colorImgR,
                         d_depthImgR,
...
//        double m_time1 = ((double)(stopTime1-startTime))/CLOCKS_PER_SEC;
            Mat trueMidImg = imread(IMG_PATH+to_string((camL+camR)/2)+"/color-cam"+to_string((camL+camR)/2)+"-f"+f_num.str()+".jpg");
